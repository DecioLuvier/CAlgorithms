...
void searchPersonByName() {
    ...
    while (LOOP_COUNTER <= WORD_LIST_LENGTH) {
        COUNTER_CHAR = (unsigned char) WORD_LIST[WORD_COUNTER_TBN];
        WORD_COUNTER_TBN += 1;
        if (strncmp(WORD_LIST + WORD_COUNTER_TBN, WORD_INPUT + 1, COUNTER_CHAR) == 0) {
            TARGET_GET = LOOP_COUNTER;
            return;
        }
        WORD_COUNTER_TBN += COUNTER_CHAR;
        COUNTER_CHAR = (unsigned char) WORD_LIST[WORD_COUNTER_TBN];
...
void removePersonByName() {
    searchPersonByName();
    if (TARGET_GET == 0 || TARGET_GET == SHRT_MAX){
    ...
    while (LOOP_COUNTER <= WORD_LIST_LENGTH) {
        if(LOOP_COUNTER == TARGET_GET){
            COUNTER_CHAR = (unsigned char) WORD_LIST[WORD_COUNTER_TBN];
            COUNTER_CHAR += 1;
            COUNTER_CHAR += (unsigned char) WORD_LIST[COUNTER_CHAR];
            COUNTER_CHAR += 1;
            COUNTER_CHAR += (unsigned char) WORD_LIST[COUNTER_CHAR];
            COUNTER_CHAR += 1;

            memmove(WORD_LIST + WORD_COUNTER_TBN, WORD_LIST + WORD_COUNTER_TBN + COUNTER_CHAR, strlen(WORD_LIST + WORD_COUNTER_TBN + COUNTER_CHAR) + 1);
        ...
        WORD_COUNTER_TBN = (unsigned char) WORD_LIST[WORD_COUNTER_TBN];
        WORD_COUNTER_TBN += 1;
        WORD_COUNTER_TBN += (unsigned char) WORD_LIST[WORD_COUNTER_TBN];
        WORD_COUNTER_TBN += 1;
        WORD_COUNTER_TBN += 1;
